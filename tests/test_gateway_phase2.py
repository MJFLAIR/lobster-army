...
# 🛡️ 戰術重啟：封鎖 DB 源頭 (Class Method 全局生效，絕對不會洩漏！)
@patch("workflows.storage.db.DB.emit_event", return_value=None)
@patch("workflows.storage.db.DB.create_task", return_value="mock_task_id")
@patch("gateway.ide_relay.Secrets")
def test_ide_relay_valid(mock_secrets, mock_create, mock_emit, client):
...
@patch("workflows.storage.db.DB.emit_event", return_value=None)
@patch("workflows.storage.db.DB.create_task", return_value="mock_task_id")
def test_webhook_valid(mock_create, mock_emit, client):
